...
public class TFGTRecipes {
	...
	public static void init(Consumer<FinishedRecipe> provider)
	{
		...
		for (int i = 0; i < TFGMachines.RAILGUN_ITEM_LOADER_IN.length; i++) {
			if (TFGMachines.RAILGUN_ITEM_LOADER_IN[i] != null && TFGMachines.RAILGUN_ITEM_LOADER_OUT[i] != null) {
				VanillaRecipeHelper.addShapedRecipe(provider, TFGCore.id("railgun_input_bus_create_" + TFGMachines.RAILGUN_ITEM_LOADER_IN[i].getTier()),
						...
						" d ", "rBx", " w ", 'B', ITEM_IMPORT_BUS[i].asStack());
				...
				VanillaRecipeHelper.addShapedRecipe(provider, TFGCore.id("railgun_input_convert_" + TFGMachines.RAILGUN_ITEM_LOADER_IN[i].getTier()),
						TFGMachines.RAILGUN_ITEM_LOADER_IN[i].asStack(),
						"d", "B", 'B', TFGMachines.RAILGUN_ITEM_LOADER_OUT[i].asStack());
				VanillaRecipeHelper.addShapedRecipe(provider, TFGCore.id("railgun_output_convert_" + TFGMachines.RAILGUN_ITEM_LOADER_OUT[i].getTier()),
						...
						"d", "B", 'B', TFGMachines.RAILGUN_ITEM_LOADER_IN[i].asStack());
			}
		}
		...
		GTRecipeTypes.ASSEMBLER_RECIPES.recipeBuilder(TFGCore.id("railgun_ammo_basic"))
				...
				.duration(20 * 10)
				.inputItems(ChemicalHelper.get(TagPrefix.plateDouble, GTMaterials.Steel).copyWithCount(2))
				.inputItems(ChemicalHelper.get(TagPrefix.rod, GTMaterials.SteelMagnetic).copyWithCount(2))
				.inputItems(ChemicalHelper.get(TagPrefix.wireFine, GTMaterials.AnnealedCopper).copyWithCount(2))
				.save(provider);
		...
		GTRecipeTypes.ASSEMBLER_RECIPES.recipeBuilder(TFGCore.id("railgun_ammo_advanced"))
				...
				.duration(20 * 10)
				.inputItems(ChemicalHelper.get(TagPrefix.plateDouble, GTMaterials.BlackSteel).copyWithCount(2))
				.inputItems(ChemicalHelper.get(TagPrefix.rod, GTMaterials.SteelMagnetic).copyWithCount(4))
				.inputItems(ChemicalHelper.get(TagPrefix.wireFine, GTMaterials.Gold).copyWithCount(2))
				.save(provider);
		...
		GTRecipeTypes.ASSEMBLER_RECIPES.recipeBuilder(TFGCore.id("railgun_ammo_best"))
				...
				.duration(20 * 10)
				.inputItems(ChemicalHelper.get(TagPrefix.plateDouble, GTMaterials.Tungsten).copyWithCount(2))
				.inputItems(ChemicalHelper.get(TagPrefix.rod, GTMaterials.NeodymiumMagnetic).copyWithCount(4))
				.inputItems(ChemicalHelper.get(TagPrefix.wireFine, GTMaterials.Aluminium).copyWithCount(2))
				.save(provider);
