...
public class MappedRivuletFeature extends Feature<BlockStateMapConfig> {
    public MappedRivuletFeature(Codec<BlockStateMapConfig> codec) {
        super(codec);
    }
    ...
    public boolean place(FeaturePlaceContext<BlockStateMapConfig> context) {
        ...
        final BoundingBox box = new BoundingBox(chunkPos.getMinBlockX() - 14, Integer.MIN_VALUE, chunkPos.getMinBlockZ() - 14, chunkPos.getMaxBlockX() + 14, Integer.MAX_VALUE,
                chunkPos.getMaxBlockZ() + 14); // Leeway so we can check outside this box
