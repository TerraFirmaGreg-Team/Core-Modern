...
public record NeedsPostProcessingSurfaceRuleSource(BlockState state, SurfaceRules.SurfaceRule fallbackRule) implements SurfaceRules.RuleSource {

	private NeedsPostProcessingSurfaceRuleSource(BlockState state) {
		this(state, (pX, pY, pZ) -> state);
	}

	public static final KeyDispatchDataCodec<NeedsPostProcessingSurfaceRuleSource> CODEC =
		KeyDispatchDataCodec.of(BlockState.CODEC.xmap(NeedsPostProcessingSurfaceRuleSource::new, NeedsPostProcessingSurfaceRuleSource::state)
			.fieldOf("state"));
