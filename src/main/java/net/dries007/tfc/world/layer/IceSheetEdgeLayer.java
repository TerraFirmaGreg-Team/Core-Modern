...
public enum IceSheetEdgeLayer implements AdjacentTransformLayer {
    ...
    public int apply(AreaContext context, int north, int east, int south, int west, int center) {
        ...
        if (center == LAKE && matcher.test(TFCLayers::isFlatIceSheet)
        ...
        if (TFCLayers.isFlatIceSheet(center) && matcher.test(i -> i == MELTWATER_LAKE)) {
            if (matcher.test(IceSheetEdgeLayer::isNotIceSheet)) {
                return SUBGLACIAL_LAKE;
            }
        }

        if (TFCLayers.isFlatIceSheet(center) && matcher.test(i -> i == OCEAN || i == OCEAN_REEF || i == DEEP_OCEAN || i == DEEP_OCEAN_TRENCH || i == ICE_SHEET_SHORE)) {
