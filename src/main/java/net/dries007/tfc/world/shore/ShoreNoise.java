	...
	public static ShoreNoiseSampler sandyBeach(Seed seed)
	{
		return new ShoreNoiseSampler()
		{
			...
			public double noise(int yIn, double noiseIn)
			{
				...
				final double heightMultiplier = Helpers.clamp((yIn - sandHeight) * 0.125, 0, 1);
	...
	public static ShoreNoiseSampler seaStacks(Seed seed)
	{
		return new ShoreNoiseSampler()
		{
			...
			public double noise(int yIn, double noiseIn)
			{
				...
				final double stackOutput = Helpers.clamp((stackNoiseValue - stackWidth) * 10, 0, 1) * Mth.clampedMap(yIn, stackBaseHeight, overhangHeight, noiseScale, 0.75);
				...
				if (landWeight >= cliffBorderTopWeight)
				{
					...
					return Math.min(Helpers.clamp((cliffBorderWeight - landWeight) * 10, 0, 1) * noiseScale, stackOutput);
				}
				else
				{
					return stackOutput;
		...
		return new ShoreNoiseSampler()
		{
			...
			public double noise(int yIn, double noiseIn)
			{
				...
				if (yIn >= lowerHeight)
				{
					...
					{
						final double cliffBorderWeight = widthFunction(true, y, height, cliffBorderBaseWeight, cliffBorderTopWeight);
						return Helpers.clamp((cliffBorderWeight - landWeight) * 10, 0, 1) * noiseScale;
					}
					else
					{
				...
				else
				{
					...
					if (oceanWeight >= cliffBorderBaseWeight)
					{
						...
						return Helpers.clamp((oceanWeight - cliffBorderWeight) * 20, 0, 1) * noiseScale;
					}
					else
					{
						return 0;
