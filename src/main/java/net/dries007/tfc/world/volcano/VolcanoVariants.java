...
public class VolcanoVariants
{
	...
	public static BlockState getStratifiedStoneBlock(int unerodedY, int y, Cellular1D cells, double rockTypeNoiseIn, Rock.BlockType blockType)
    {
        ...
        if (cell.noise() > 0.3)
        {
			if (blockType == Rock.BlockType.RAW)
				return Blocks.TUFF.defaultBlockState();
			else if (blockType == Rock.BlockType.GRAVEL)
				return TFGBlocks_Earth.TUFF_GRAVEL.getDefaultState();
			else
				throw new Exception("Missing tuff rock");
        }
